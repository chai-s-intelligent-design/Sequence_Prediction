...
- "滑动窗口"：采用滑动窗口来控制模型基于历史信息的输入。
  ...
  自回归模型（AR）的基本思想是：一个时间点的标签值可以由其过去某个时间段内的所有标签值的线性组合表示:
...
$sequence_length_of_j$
$sequence__length__of__j$
...
### 输出的损失函数
考虑到模型在每个时间步数的输出需要与对应的指示向量做损失，指示向量可以看做是一个多标签的二分类的问题，即有51个类别需要进行判断，但是每一个指示向量只有$max\_selected$个1，其余都为0，此时正负样本不平衡，并且有些样本难以分类，有些样本容易分类，因此采用**focal_loss**作为模型的损失函数，其表达式为:
